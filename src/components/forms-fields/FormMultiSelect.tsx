...
export default function FormMultiSelectCombobox<TFormValues extends FieldValues>({
    control,
    name,
    label,
    description,
    ...
    return (
        <FormField
            ...
            name={name}
            render={({ field }) => {
                const selectedValues: (string | number)[] = Array.isArray(field.value) ? field.value as (string | number)[] : [];
                const selectedOptions = getSelectedOptions(selectedValues);
